window.addEvent("domready",function(){
	$ES(".gk_tabmods").each(function(el,i){
		...
		var $G = $Gavick["gk_tabmods"+module_id]; 
		...
		var timer = false;
		if($G["styleType"] == 1){
			var baseWidth = $E(".gk_tabmods_container2", el).getSize().size.x;
			el.setStyle("width",baseWidth+"px");
			var listTab = $E('.gk_tabmodsmenu_ul',el);
			...
			baseWidth -= $E(".gk_tabmods_container0",el).getStyle("margin-left").toInt();
			baseWidth -= $E(".gk_tabmods_container0",el).getStyle("margin-right").toInt();
			$E(".gk_tabmods_container1",el).setStyle("width",baseWidth+"px");
			$E(".gk_tabmods_container0",el).setStyle("width",baseWidth+"px");
			$ES(".gk_tabmods_item", el).setStyle("width",baseWidth+"px");
		}
		$E('.gk_tabmodsmenu_ul li',el).toggleClass("active");
		var param = ($G["animationType"] == 1) ? "width": "height";
		$E(".gk_tabmods_container2", el).setStyle(param, ((amount+1)*$E(".gk_tabmods_container1", el).getSize().size.x));
		$ES(".gk_tabmods_item", el).each(function(e){e.setStyle("width", $E(".gk_tabmods_container1", el).getSize().size.x + "px");});
		$ES('.gk_tabmodsmenu_ul li', el).each(function(elm,j){
			elm.addEvent(evnt,function(){
			    actual = gk_tabmods_anim(j, actual, amount, modsArray, el, $G["animationTransition"], $G["animationSpeed"]);
				
				$ES('.gk_tabmodsmenu_ul li', el).each(function(elmt){elmt.setProperty("class","");});
				$ES('.gk_tabmodsmenu_ul li', el)[actual].toggleClass("active");
				
				if(timer){
					$clear(timer);	
					timer = (function(){
						actual = gk_tabmods_anim("right" , actual, amount, modsArray, el, $G["animationTransition"], $G["animationSpeed"]);
						$ES('.gk_tabmodsmenu_ul li', el).each(function(elmt, i){elmt.setProperty("class","");});
						$ES('.gk_tabmodsmenu_ul li', el)[actual].toggleClass("active");
		...
		if($E(".gk_tabmods_button_next", el)){
			$E(".gk_tabmods_button_next", el).addEvent("click",function(){
				actual = gk_tabmods_anim('right', actual, amount, modsArray, el, $G["animationTransition"], $G["animationSpeed"]);
				$ES('.gk_tabmodsmenu_ul li', el).each(function(elmt){elmt.setProperty("class","");});
				$ES('.gk_tabmodsmenu_ul li', el)[actual].toggleClass("active");
		...
		if($E(".gk_tabmods_button_prev", el)){
			$E(".gk_tabmods_button_prev", el).addEvent("click",function(){
				actual = gk_tabmods_anim('left', actual, amount, modsArray, el, $G["animationTransition"], $G["animationSpeed"]);	
				$ES('.gk_tabmodsmenu_ul li', el).each(function(elmt){elmt.setProperty("class","");});
				$ES('.gk_tabmodsmenu_ul li', el)[actual].toggleClass("active");
...
function gk_tabmods_anim(direct, actual, amount, modsArray, el, t, s){
	var transitions_tab = [
	...
	var scr = new Fx.Scroll($E(".gk_tabmods_container1",el), {duration: s, wait: true, transition: transitions_tab[t]});
