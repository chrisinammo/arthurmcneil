...
class JCalProControllerEvents extends JController {
    ...
    function execute($task=null) {
        $id    = JRequest::getVar( 'id', 0, $_REQUEST );
    ...
    */
    function showEvents ( $option, $section )
    {
        ...
        $mosConfig_list_limit = $registry->list_limit;
        ...
        $lists['categories']     = JHTML::_('select.genericlist', $categoriesList, 'cat', 'class="inputbox" size="1" onchange="document.adminForm.submit( );"','cat_id', 'cat_name', $cat );
    ...
    */
    function editEvent ( $id, $option, $section )
    {
        ...
        $checked['recurNone'] = ( ( int ) $row->recur_type_select == 0 ) ? 'checked' : '';
        ...
        $categoriesList[]     = JHTML::_( 'select.option', '0', 'Select Category', 'cat_id', 'cat_name' );
        $categoriesList     = array_merge ( $categoriesList, $database->loadObjectList ( ) );
        $lists['categories']     = JHTML::_('select.genericlist',  $categoriesList, 'cat', 'class="inputbox" size="1"','cat_id', 'cat_name', $row->cat );
        ...
        {
            $hourList[] = JHTML::_( 'select.option', sprintf ( "%02d" ,$i ), sprintf ( "%02d", $i ), 'hour_id', 'hour_name' );
        }

        $lists['hours']     = JHTML::_('select.genericlist',  $hourList, 'hours', 'class="inputbox" size="1"','hour_id', 'hour_name', $startDateArr['hours'] );
        ...
        if( !$CONFIG_EXT['time_format_24hours'] )
        {
            $ampmList[] = JHTML::_( 'select.option', 'am', 'am', 'ampm_id', 'ampm_name' );
            $ampmList[] = JHTML::_( 'select.option', 'pm', 'pm', 'ampm_id', 'ampm_name' );

            $lists['ampm'] = JHTML::_('select.genericlist',  $ampmList, 'ampm', 'class="inputbox" size="1"','ampm_id', 'ampm_name', $row->start_time_ampm );
        }
        else
        {
            $lists['ampm'] = '';
        ...
        $recurValuesList[] = JHTML::_( 'select.option', '', 'Pick Time', 'recur_id', 'recur_name' );
        $recurValuesList[] = JHTML::_( 'select.option', 'day', 'Day(s)', 'recur_id', 'recur_name' );
        $recurValuesList[] = JHTML::_( 'select.option', 'week', 'Week(s)', 'recur_id', 'recur_name' );
        $recurValuesList[] = JHTML::_( 'select.option', 'month', 'Month(s)', 'recur_id', 'recur_name' );
        $recurValuesList[] = JHTML::_( 'select.option', 'year', 'Year(s)', 'recur_id', 'recur_name' );

        $lists['recurValues'] = JHTML::_('select.genericlist',  $recurValuesList, 'recur_type', 'class="inputbox" size="1"','recur_id', 'recur_name', $row->recur_type );

        // building recuring day list
        for ( $i = 1; $i <= 31; $i++ )
        {
            $recuringDaysList[] = JHTML::_( 'select.option', $i, $i, 'recuringday_id', 'recuringday_name' );
        }

        $lists['recuringDays']     = JHTML::_('select.genericlist',  $recuringDaysList, 'recuringDay', 'class="inputbox" size="1"','recuringday_id', 'recuringday_name', $recurUntilArr['mday'] );

        // building recuring month list
        for ( $i = 1; $i <= 12; $i++ )
        {
            $recuringMonthList[] = JHTML::_( 'select.option', $i, $lang_date_format['months'][$i-1], 'recuringMonth_id', 'recuringMonth_name' );
        }

        $lists['recuringMonths']     = JHTML::_('select.genericlist',  $recuringMonthList, 'recuringMonth', 'class="inputbox" size="1"','recuringMonth_id', 'recuringMonth_name', $recurUntilArr['mon'] );
        ...
        for ( $i = 1; $i <= 4; $i++ )
        {
            $recuringYearList[] = JHTML::_( 'select.option', $y, $y, 'recuringYear_id', 'recuringYear_name' );

            $y++;
        }

        $lists['recuringYears']     = JHTML::_('select.genericlist',  $recuringYearList, 'recuringYear', 'class="inputbox" size="1"','recuringYear_id', 'recuringYear_name', $recurUntilArr['year'] );
    ...
    */
    function saveEvent ( $option, $section )
    {
        ...
        $mainframe->redirect( "index2.php?option=$option&section=$section" );
    }
