...
var adminVisualizationCalendar = new Class({
	initialize: function(){
		...
		this.changeConnectionclick = this.changeConnection.bindAsEventListener(this);
		this.addDataGroupClick = this.addDataGroup.bindAsEventListener(this);
	...
	deleteConnection: function(event){
		...
		this.watchDelete();
		e.target.getParent().getParent().getParent().getParent().remove()
		this.watchConnections();
		e.stop();
	},
	
	makeDataGroup: function(cnnId, tblName, fldDate, fldLabel, keyLabel, colour){
		...
		var fieldList1 = fldDate ? this._makeFieldDd(cnnId, tblName, fldDate, 'params[table_date][]') : this.translate.selectTable;
		...
		table.injectInside($('datesContainer'));
		this.counter ++;
		this.watchDelete();
		this.watchConnections();
	},
	...
	changeTable: function(tbl){
		var cid = tbl.getParent().getParent().getPrevious().getChildren()[1].getChildren()[0].getValue();
		var targ = tbl.getParent().getParent().getNext().getChildren()[1];
		var targ2 = tbl.getParent().getParent().getNext().getNext().getChildren()[1];
		var tid = tbl.getValue();
		var url = 'index.php?option=com_fabrik&c=table&class=table&format=raw&task=ajax_loadTableDropDown&cid=' + cid + '&table=' + tid + '&name=params[table_date][]' ;
