...
Plotr.Base.K= function(x) { return x };
	    ...
	    if(this.options.drawYAxis){
	        if(this.yticks){
				var collectYLabels = function(tick){
					...
					var label = new Element('div', {'class':'yaxis'});
		...
		if(this.options.drawXAxis){
	        if(this.xticks){
				var collectXLabels = function(tick){
	                ...
	                var label = new Element('div', {'class':'xaxis'});
	                label.appendText(tick[1]);
...
Plotr.Chart = {
	...
	addLegend: function(/*String*/ id){
		...
		element=new Element('fieldset',{'styles':{backgroundColor:'#fff',width:wd}}).adopt(new Element('legend').appendText('Legend')).adopt(ul);
