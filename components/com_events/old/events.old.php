 ...
 *
 * @version     $Id: events.old.php 964 2008-02-16 11:00:03Z geraint $
 * @package     Events
...
$db	=& JFactory::getDBO();
$offset 		= intval( JRequest::getVar('offset',0 ) );
...
function showEventsByWeek( $year, $month, $day ){
	...
	for( $d = 0; $d < 7; $d++ ){
		...
		$link = JRoute::_( 'index.php?option=' . $option . '&amp;task=view_day&amp;year=' . $week_year
		...
		if( $week_month == strftime( '%m', time() + ( $mainframe->getCfg('offset') * 60 * 60 ) )
		&& $week_year == strftime( '%Y', time() + ( $mainframe->getCfg('offset') * 60 * 60 ) )
		&& $week_day == strftime( '%d', time() + ( $mainframe->getCfg('offset') * 60 * 60 ) )
		) {
        ...
        <table width="95%" align="center" border="0" cellspacing="1" cellpadding="0" class="cal_table">
			...
			<tr valign="top" style="height:80px;">
                ...
                for( $d = 1; $d <= date( 't', mktime( 0, 0, 0,( $month + 1 ), 0, $year )); $d++ ){
                	...
                	if( $month == strftime( '%m', time() + ( $mainframe->getCfg('offset') * 60 * 60 ))
                	&& $year == strftime( '%Y', time() + ( $mainframe->getCfg('offset') * 60 * 60 ))
                	&& $d == strftime( '%d', time() + ( $mainframe->getCfg('offset') * 60 * 60 ))) {
                	...
                	$link = JRoute::_( 'index.php?option=' . $option . '&amp;task=view_day&amp;year='
                        	...
                        	for( $i = 0; $i < $cellcount; $i++ ){
                        		...
                        		if( $checkprint->viewable == true ){
                        			...
                        			*/
                        			...
                        			require( JPATH_SITE . '/components/' . $option
	...
	if( $num_events > 0 ){
		for( $r = 0; $r < $num_events; $r++ ) {
			...
			if( $event_day_month_year <> $chdate ){
				echo '<tr>';
				if( $event_up->month == strftime( '%m', time() + ( $mainframe->getCfg('offset')*60*60 ) )
				&& $event_up->year == strftime( '%Y', time() + ( $mainframe->getCfg('offset')*60*60 ) )
				&& $event_up->day == strftime( '%d', time() + ( $mainframe->getCfg('offset')*60*60 ) )
				) {
					$bg = ' class="ev_td_today"';
				} else {
